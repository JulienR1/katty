...
export class PlayCommand extends Command {
    ...
    public async execute({ keywords, member, channel }: ICommandDescription) {
        ...
        try {
            ...
            const trackPromises: Promise<ITrack[]>[] = Object.values(factories).map((factory) => factory.prototype.from(searchArg));
            const tracks: ITrack[] = await Promise.any(trackPromises);

            const playerLibrary = PlayerLibrary.Instance();
            const voiceChannel = member?.voice.channel as VoiceChannel;
            (playerLibrary.getFrom(voiceChannel) || (await playerLibrary.addTo(voiceChannel, () => this.goodbyeEmbed(channel, voiceChannel)))).enqueue(tracks);
