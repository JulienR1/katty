...
export class MoveCommand extends Command {
  ...
  private successEmbed = (song: string, position: number) => new SuccessEmbed().setTitle(i18n.__("move.title", { song, position: position.toString() }));

  private errorEmbed = (startIndex: number, targetIndex: number) => new ErrorEmbed().setDescription(i18n.__("move.error", { startIndex: startIndex.toString(), targetIndex: targetIndex.toString() }));
}
